...
const allUpperCase = (arr) => {
  // Solution code here...
  const newArr=[];
  arr.forEach((upper)=>newArr.push(upper.toUpperCase()))
  return newArr;
};
...
const speaker = (words, callback) => {
  // Solution code here...
  const newArr=[];
  words.forEach((strings)=> newArr.push(callback(strings)));
  return newArr;
};
...
const createList = (availableItems) => {
  // Solution code here...
const availableGrocery =[];
availableItems.forEach((value)=>{
  if (value.available==true){
      availableGrocery.push(value.name);
  }
})
